...
def read_json_basic(file_path: str) -> pd.DataFrame:
    ...
    json = pd.read_json(file_path, lines = True)
    return json

def read_json_with_orientation(file_path: str, orient: str) -> pd.DataFrame:
    # Write here your code
    json = pd.read_json(file_path, orient = orient)
    return json

def read_json_with_table(file_path: str, orient: str) -> pd.DataFrame:
    # Write here your code
    json =  read_json_with_orientation(json_table_oriented_path, orient='table')
    return json
...
df_basic = read_json_basic(json_basic_path)
df_orient = read_json_with_orientation(json_orient_path, orient="records")
df_table_oriented = read_json_with_table(json_table_oriented_path, orient="table")
df_normalized = read_json_and_normalize(json_normalize_path, record_path=["data"])
